...
if __name__ == '__main__':
	...
	outputtxt = args.rawdir + "/out/output.txt"

	rcon = Rcon('localhost', args.port, args.password)
	...
	class MyEvent(pyinotify.ProcessEvent):
		def process_IN_MODIFY(self,event):
			...
			with eventlet.Timeout(0.5,False):
				response = rcon.send_command(request)
				print(response)
				os.system('echo "' + response + '" > ' + outputtxt)
#			print("Error4 Error4 Error4 Error4 Error4 Error4 Error4 Error4 Error4 Error4")
#			os.system('echo Error4 Error4 Error4 Error4 Error4 Error4 Error4 Error4 Error4 Error4 > ' + outputtxt)
	def main(inputtxt):
		vm = pyinotify.WatchManager()
		vm.add_watch(inputtxt,pyinotify.IN_MODIFY)
		event = MyEvent()
