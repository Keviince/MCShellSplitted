...
if __name__ == '__main__':
	...
	class MyEvent(pyinotify.ProcessEvent):
		def process_IN_MODIFY(self,event):
			DMname = str.strip(os.popen('tail -1 ' + inputtxt + ' | cut -d ":" -f 1 | cut -d "-" -f 3- | sed s/[[:space:]]//g').read())
			DMcontent = str.strip(os.popen('tail -1 ' + inputtxt + ' | cut -d ":" -f 2- | sed "s/^[ \t]*//g"').read())	
			request = 'tellraw ' + args.player + ' {\"text\":\"<' + DMname + '(BiliBili)> ' + DMcontent + '\"}"'
			print(request)
			rcon.send_command(request)
	def main(inputtxt):
